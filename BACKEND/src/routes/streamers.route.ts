...
router.get(
  ...
  async (req: Request, resp: Response) => {
    try {
      ...
      if (perfil) {
        total_minutos = perfil.horas_transmitidas_total; // La columna guarda minutos
      } else {
      ...
      // 2. Convertir a HORAS una sola vez
      ...
      return resp.status(500).json({ error: "Error interno" });
    }
  }
);
