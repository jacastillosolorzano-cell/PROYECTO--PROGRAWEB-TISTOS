...
import { ArrowLeft, Gift, Wallet, Coins, ChevronRight } from "lucide-react";
import BottomNav from "@/components/BottomNav";
import React from 'react';
import ComprobanteModal from '../components/ComprobanteModal';
import PaymentModal from '../components/PaymentModal'; // Nuevo import
  ...
  const handleSubmitPayment = (event: React.FormEvent) => {
    ...
    if (rechargeAmount > 0 && cardNumber && expiryDate && cvc) {
      ...
      setTimeout(() => {
        ...
        setMonedas(monedas + rechargeAmount);
      ...
      <div className="max-w-xl mx-auto bg-card rounded-xl p-6 mb-4 shadow flex flex-col gap-2">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-2">
            <Button variant="link" size="sm" onClick={() => { handleComprarMonedas(); realizarCompra(); }}>
              Obtener Monedas
            </Button>
        ...
        <div className="flex items-center justify-left gap-4">
          ...
          <span className="text-2xl font-bold">{monedas} SOLES</span>
        </div>
      </div>
