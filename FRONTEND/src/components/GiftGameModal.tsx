...
const multipliers = [1, 1, 1, 2, 2, 3]; // distribución simple (más probabilidad de x1)
...
const GiftGameModal: React.FC<Props> = ({ open, gift, onClose, onResult }) => {
  ...
  const wheelItems = useMemo(() => {
    return multipliers.map((m, i) => ({ id: i, label: `${m}x` }));
  }, []);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h3 style={{ margin: 0, marginBottom: 8 }}>Mini‑juego: multiplicador</h3>
        <p style={{ marginTop: 0, marginBottom: 12, color: "#cfc" }}>
          Regalo: <strong>{gift.nombre}</strong> — costo base: {gift.costo} monedas
        </p>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
          ...
          <button onClick={spin} disabled={spinning} style={{
            ...
            {spinning ? "Jugando..." : (result ? `Aceptar ${result}x` : "Jugar (Girar)")}
          </button>
        </div>
      </div>
    </div>
